...
async function getLatestPathPlusOne(skipId) {
  ...
  return latestPathWithCount ? Math.abs(Number(latestPathWithCount)) + 1 : 0;
}
...
module.exports = () => ({
  ...
  create: async (data) => {
    ...
    if (!data.url_path) {
      ...
      defaultExt = await getLatestPathPlusOne();
    }
    ...
    await duplicateCheck(defaultExt);
  ...
  update: async (id, data) => {
    ...
    if (!data.url_path) {
      // data.url_path will holds the auto generated path
      // if data.url_path = null, auto use latest count +1
      defaultExt = await getLatestPathPlusOne(id);
    }
